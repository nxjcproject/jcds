    ...
    if ("first" == type) {
        $("#grid_tzReportDatagrid").datagrid({
            ...
            columns: [[
		                ...
		                { field: 'Date', title: '报表日期', width: 80 },
                        //{
...
function loadtformulaDatagrid(type, columnsData, rowData) {
    if ("first" == type) {
...
function researchFun() {
    ...
    if ("year" == mValue) {
        myear = $("#date_year").val();
        urlString = "table_FormulaHeatmeter.aspx/GettzHeatmeterJsonbyYear";
        ...
        urlString = "table_FormulaHeatmeter.aspx/GettzHeatmeterJsonbyStartEndDate";
...
function LoadformulaDatagridData(rowInfo) {
    ...
    $.ajax({
        type: "POST",
        url: "table_FormulaHeatmeter.aspx/GetformulaDatagridDataJson",
        ...
        success: function (msg) {
            ...
            if (myData.total == 0) {
                ...
                loadtformulaDatagrid("last", myData.columns, myData.rows);
            }
        },
        beforeSend: function (XMLHttpRequest) {
            //alert('远程调用开始...');
